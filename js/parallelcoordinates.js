function ParallelCoordinates(data,options) {
		...
		return d3.nest()
			...
			.rollup(function(leaves) {
				...
				options.columns.forEach(function(col){
					...
					r.name=leaves[0]["language_name"];
					r.name2=leaves[0]["language_name"];
				})
				return r;
			})
			.entries(data)
			...
			.sort(function(a,b){
				return d3.descending(a.values["sum"],b.values["sum"]);
			})
			//.slice(0,29)
	}
	...
	HEIGHT=Math.min(Math.max(Math.round(window.innerHeight-150),920),900);
	...
	this.loadData=function(quarter) {
		...
		d3.json("data/quantity.json",function(q){
			...
			for (var i = 0; i < q.length; i++){
				...
				q.year=programming_languages[obj.language_name.toLowerCase()] || 1970;
			...
			nested_data=nestData(d3.values(data));
